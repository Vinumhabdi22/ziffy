        ...
        <section className="mb-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-6">
                <div>
                    ...
                    <p className="text-warm-gray-500 text-lg mt-1">{details.quickStats.propertyType} • {listing.county || 'County'}</p>
                </div>
                <div className="flex flex-col items-end gap-2 mt-7">
                    <p className="text-primary text-3xl md:text-4xl font-black leading-tight tracking-[-0.033em]">{listing.price}</p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 h-[400px] mb-8 rounded-xl overflow-hidden">
                <div className="col-span-1 md:col-span-2 md:row-span-2 relative group cursor-pointer h-full">
                    <div
                        ...
                        style={{ backgroundImage: `url('${details.gallery[0]}')` }}
                    ></div>
                ...
                <div className="relative group cursor-pointer h-full min-h-[190px]">
                    <div
                        className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
                        style={{ backgroundImage: `url('${details.gallery[1]}')` }}
                    ></div>
                </div>
                <div className="relative group cursor-pointer h-full min-h-[190px]">
                    <div
                        className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
                        style={{ backgroundImage: `url('${details.gallery[2]}')` }}
                    ></div>
                </div>
                <div className="relative group cursor-pointer h-full min-h-[190px]">
                    <div
                        className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
                        style={{ backgroundImage: `url('${details.gallery[3]}')` }}
                    ></div>
                </div>
                <div className="relative group cursor-pointer h-full min-h-[190px]">
                    <div
                        className="absolute inset-0 bg-cover bg-center transition-transform duration-500 group-hover:scale-105"
                        style={{ backgroundImage: `url('${details.gallery[4] || details.gallery[0]}')` }}
                    ></div>
                    <div className="absolute inset-0 bg-black/50 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                        <span className="text-white font-medium flex items-center gap-1">
                            <span className="material-symbols-outlined">grid_view</span> View All {details.gallery.length} Photos
                        </span>
                    </div>
                </div>
            </div>
